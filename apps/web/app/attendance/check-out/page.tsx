...
export default function CheckOutPage() {
  ...
  // Mock check-in time (in a real app, this would be fetched from the server)
  const checkInTime = new Date();
  checkInTime.setHours(9, 0, 0);

  // Calculate work hours
  const workHours = ((currentTime.getTime() - checkInTime.getTime()) / (1000 * 60 * 60)).toFixed(2);
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      // After successful check-out, redirect to attendance page after 2 seconds
      setTimeout(() => {
    ...
    <DashboardLayout>
      <div className="container mx-auto py-6">
        <div className="max-w-md mx-auto">
          <h1 className="text-3xl font-bold tracking-tight mb-6">Check Out</h1>
          
          <Card>
            <CardHeader>
              <CardTitle>End Your Workday</CardTitle>
              <CardDescription>
                Check out for today, {format(currentTime, 'EEEE, MMMM d, yyyy')}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              {isCheckedOut ? (
                <div className="flex flex-col items-center justify-center py-6 text-center">
                  ...
                  <h3 className="mt-4 text-lg font-semibold text-green-600">Successfully Checked Out!</h3>
                  <p className="mt-2 text-sm text-gray-500">
                    You have checked out at {format(currentTime, 'h:mm:ss a')}
                  </p>
                  <p className="mt-1 text-sm text-gray-500">
                    Work hours today: {workHours} hours
                  </p>
                  <p className="mt-1 text-sm text-gray-500">
                ...
                <>
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <div className="text-lg font-semibold">{format(checkInTime, 'h:mm a')}</div>
                    </div>
                    ...
                    <div>
                      <div className="text-sm text-gray-500 mb-1">Work hours</div>
                      <div className="text-lg font-semibold">{workHours} hrs</div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      id="notes"
                      placeholder="Any notes for today's check-out?"
                      value={notes}
            ...
            {!isCheckedOut && (
              <CardFooter className="flex justify-between">
                ...
                <Button 
                  ...
                  disabled={isLoading || locationStatus === 'loading' || !summary.trim()}
                >
